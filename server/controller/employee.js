...
const employeeCtrl = {
    getAllEmployees: async (req, res) => {
        try {
            ...
            res.status(200).json({
                msg: 'Successfully get all employees',
                employee,
            })
        } catch (error) {
        ...
        try {
            Employee.findOne({ _id: req.params.id }, async (err, employee) => {
                if (err) {
                    return res
                        .status(400)
                        .json({ msg: 'Employee does not exist' })
                } else {
                    res.status(200).json({
                        msg: 'Successfully get employee',
                        employee,
                    })
                }
            }).select('-password')
    ...
    updateEmployee: async (req, res) => {
        try {
            ...
            if (!employee) {
                return res
                    .status(404)
                    .json({ msg: `no employee with id ${id}` })
            }
            return res.status(200).json({ msg: 'Update Success', employee })
        } catch (error) {
            return res.status(500).json({ msg: error.message })
        }
    },
                ...
                async (err, employee) => {
                    if (err) {
                        ...
                        if (employee.isAdmin === true) {
                            res.status(200).json({
                                msg: 'Admin has successfully updated the password',
                                employee,
                            })
                        } else {
                            res.status(200).json({
                                msg: 'Employee has successfully updated the password',
                                employee,
                            })
                        }
                    }

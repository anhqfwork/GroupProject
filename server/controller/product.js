...
const productCtrl = {
  ...
  createProduct: async (req, res) => {
    try {
      const { title, category, publisher, price, inStock, isbn, authorName, image } = req.body
      // if (!image) {
      ...
      res.status(201).json({ msg: 'Create a new product', book: newProduct })
    } catch (error) {
  ...
  updateProduct: async (req, res) => {
    try {
      ...
      return res.status(200).json({ msg: 'Successfully updated book', book })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
  ...
  filterProduct: async (req, res) => {
    try {
      ...
      // const reqexp = new RegExp('^' + name)
      const product = await Product.find({ price: { $lt: priceSmaller, $gt: priceLarger }, publisher })
      console.log(product)
      if (!product) {
        return res.status(400).json({ msg: 'Product does not exist' })
      }
      res.status(200).json({ msg: 'Filter product successfully', product })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
    ...
    try {
      ...
      if (product) {
        ...
        if (product.numberOfReviews == 0) {
          ...
          product.ratings = (product.ratings * product.numberOfReviews + rating) / (product.numberOfReviews + 1)
        }
        product.numberOfReviews += 1

        await product.save()
        return res.status(200).json({ msg: 'Successfully created the review', product })
      } else {
        return res.status(404).json({ msg: 'Product does not exist' })
      }
    } catch (error) {
  ...
  updateProductReview: async (req, res) => {
    try {
      ...
      const product = await Product.findOneAndUpdate(
        { _id: mongoose.Types.ObjectId(id), reviews: { $elemMatch: { userId: mongoose.Types.ObjectId(userId) } } },
      ...
      await product.save()
      return res.status(200).json({ msg: 'Successfully updated product review', product })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
  ...
  deleteProductReview: async (req, res) => {
    try {
      ...
      const userId = req.userId
      const id = req.params.id
      const product = await Product.findOneAndUpdate({ _id: id }, { $pull: { reviews: { userId: mongoose.Types.ObjectId(userId) } } }, { new: true })
      const reviews = product.reviews
      if (reviews.length !== 0) {
        for (let i = 0; i < reviews.length; i++) {
          totalRating += reviews[i].rating
  ...
  getTop10Products: async (req, res) => {
    try {
      ...
      return res.status(200).json({ msg: 'Top 10 rated products', products })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
  ...
  getAllReviews: async (req, res) => {
    try {
      const id = req.params.id
      const book = await Book.findOne({ _id: id }, 'reviews').select('-_id')
      if (!book) {
  ...
  getFiveNewestProducts: async (req, res) => {
    try {
      ...
      return res.status(200).json({ msg: 'Successfully get 5 newest products', product })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
  ...
  getMostSoldProducts: async (req, res) => {
    try {
      const order = await Order.aggregate([
        ...
        // { $unwind: '$orderItems.product' },
        {$group:{
          _id:"$orderItems.product", quantity:{$sum:'$orderItems.quantity'}
        }},
        {
          $lookup: {
            from: 'products',
      ...
      return res.status(200).json({ msg: 'Successfully get 5 most sold products', order })
    } catch (error) {
      return res.status(500).json({ msg: error.message })
    }
  },
